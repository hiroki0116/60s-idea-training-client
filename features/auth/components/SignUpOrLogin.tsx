...
const SignUpOrLogin = () => {
  const { setShowLoginOrRegister, setShowLogin, setShowRegister, setUser } =
    useContext(AuthContext);
  ...
  const handleSubmit = async (formValues) => {
    ...
    try {
      ...
      const { data } = await APIWithoutAuth.get(
        `/users/?email=${formValues.email.toLowerCase()}`
      );
      setShowLoginOrRegister(false);
      if (data.success) {
        setShowLogin(true);
    ...
    <Spin spinning={loading}>
      <div className="p-8">
        ...
        >
          ...
          >
            <Input
              className="p-2 rounded-lg font-dark-gray text-16"
              placeholder="Email"
            />
          </Form.Item>
          <Form.Item>
